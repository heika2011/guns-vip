...
public class LoginController extends BaseController {
    ...
    public String index(Model model) {
        ...
        if (LoginContextHolder.getContext().hasLogin()) {
            Map<String, Object> userIndexInfo = userService.getUserIndexInfo(null);

            //用户信息为空，提示账号没分配角色登录不进去
            if (userIndexInfo == null) {
                model.addAttribute("tips", "该用户没有角色，无法登陆");
                return "/login.html";
            }else if(userIndexInfo.get("menus")!=null){
                List<MenuNode> menusByRoleIds = menuService.getMenusByRoleIds(LoginContextHolder.getContext().getUser().getRoleList(), null);
                if (menusByRoleIds.size()==0) {
                    model.addAttribute("tips", "该用户后台没有角色，无法登陆");
                    return "/login.html";
                }else {
