...
public class OAuthRequestFactory {
     ...
     * 服务器基础地址
     */
    private static final String BASE_URL = "http://xiexinxin.ngrok2.xiaomiqiu.cn";
        ...
        switch (source) {
            ...
            case "wechat":
                authRequest = new AuthWeChatRequest(AuthConfig.builder()
                        .clientId(ConstantsContext.getWeChatClientId())
                        .clientSecret(ConstantsContext.getWeChatSerect())
                        .redirectUri(ConstantsContext.getWeChatKxUrl() + "/oauth/callback/wechat?render={URL}&type={TYPE}")
                        .build());
                break;
            case "csdn":
