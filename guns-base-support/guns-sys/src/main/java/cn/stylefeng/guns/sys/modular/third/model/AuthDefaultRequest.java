...
public abstract class AuthDefaultRequest implements AuthRequest {
    ...
    protected AuthSources source;

    public AuthDefaultRequest(AuthConfig config, AuthSources source) {
        this.config = config;
        this.source = source;
    }
     ...
     * @return token
     * @see me.zhyd.oauth.request.AuthDefaultRequest#authorize()
     * @see me.zhyd.oauth.request.AuthDefaultRequest#authorize(String)
     */
    ...
    /**
     * 统一的登录入口。当通过{@link me.zhyd.oauth.request.AuthDefaultRequest#authorize(String)}授权成功后，会跳转到调用方的相关回调方法中
    ...
    /**
     * 处理{@link me.zhyd.oauth.request.AuthDefaultRequest#login(AuthCallback)} 发生异常的情况，统一响应参数
     *
     * @param e 具体的异常
     * @return AuthResponse
     */
     ...
     * 建议使用{@link me.zhyd.oauth.request.AuthDefaultRequest#authorize(String)}方法生成授权地址，在回调方法中对{@code state}进行校验
     *
     * @return 返回授权地址
     * @see me.zhyd.oauth.request.AuthDefaultRequest#authorize(String)
     */
    ...
    protected String userInfoUrl(AuthToken authToken) {
        String s = UrlBuilder.fromBaseUrl(source.userInfo()).queryParam("access_token", authToken.getAccessToken()).queryParam("code",authToken.getCode()).build();
