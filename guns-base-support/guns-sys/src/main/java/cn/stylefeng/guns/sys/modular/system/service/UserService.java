...
import cn.stylefeng.guns.sys.modular.system.model.entity.UserPos;
...
public class UserService extends ServiceImpl<UserMapper, User> {
    ...
    public void deleteUser(Long userId) {
        ...
        //删除对应的oauth2绑定表
        this.baseMapper.removeOauth(userId);
        //删除职位关联
    ...
    public int setRoles(Long userId, String roleIds) {
        ...
        z.setUserId(userId);
        //z.setSee(0);
        this.baseMapper.updateById(z);
        if(split!=null){
            for(String s:split){
                if(roleMapper.selectOne(new QueryWrapper<Role>().eq("role_id",s).eq("see",1))!=null){
                    z.setSee(1);
                    this.baseMapper.updateById(z);
                }
            }
        ...
        if(roleMapper.selectOne(new QueryWrapper<Role>().eq("role_id",roleIds).eq("see",1))!=null){
            z.setSee(1);
            this.baseMapper.updateById(z);
        }
        return this.baseMapper.setRoles(userId, roleIds);
     ...
     * @Date 2018/12/24 22:46
     */
    public List<Map<String, Object>> getUserMenuNodes(List<Long> roleList,String type) {
        if (roleList == null || roleList.size() == 0) {
            return new ArrayList<>();
        } else {
            List<MenuNode> menus = menuService.getMenusByRoleIds(roleList,type);
     ...
     */
    public Map<String, Object> getUserIndexInfo(String type) {
        ...
        List<Map<String, Object>> menus = this.getUserMenuNodes(roleList,type);
     ...
     * @return
     */
    public List<String> getAccountByRoleId(String userName) {
        return baseMapper.getAccountByRoleId(userName);
    }

    public List<MenuNode> getUserMenu() {

        LoginUser user = LoginContextHolder.getContext().getUser();
        List<Long> roleList = user.getRoleList();
        List<MenuNode> userMenu = menuService.getOutMenusByRoleIds(roleList);
        return userMenu;
    }
