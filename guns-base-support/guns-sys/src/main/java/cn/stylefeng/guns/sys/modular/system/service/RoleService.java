...
import cn.stylefeng.guns.sys.modular.system.model.entity.Relation;
...
public class RoleService extends ServiceImpl<RoleMapper, Role> {
     ...
     */
    @Transactional
    public void updateSee(Map param) {

        JSONArray role = (JSONArray)param.get("role");
        Role r = new Role();
        User u = new User();
        JSONArray user = (JSONArray)param.get("user");
        u.setSee(0);
        userMapper.update(u,new QueryWrapper<User>().ne("user_id",1));
        if(user.size()!=0){
            for(int i=0;i<user.size();i++){
                u.setSee(1);
                userMapper.update(u,new QueryWrapper<User>().eq("user_id",user.get(i)));
            }
        }
        u.setSee(1);
        r.setSee(0);
        this.baseMapper.update(r, new QueryWrapper<Role>().eq("pid","1234732820551712770"));
        if(role.size()!=0){
            for(int i=0;i<role.size();i++){
                r.setSee(1);
                this.baseMapper.update(r,new QueryWrapper<Role>().eq("role_id",role.get(i)));
                userMapper.update(u,new QueryWrapper<User>().like("role_id",role.get(i)));
            }
        }
