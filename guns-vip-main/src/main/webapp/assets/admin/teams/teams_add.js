...
layui.use(['form', 'admin', 'ax','element','table','func'], function () {
    ...
    form.on('submit(btnSubmitChild)', function (data) {
        var operation = function () {
            ...
            {
                milasUrl.push({
                    userId: tableData[key][0].split("|")[1],
                    name: tableData[key][0].split("|")[0],
                    job: tableData[key][1],
                });//将每一行存入对象
            }
            var dataz = {name:$("input[name='name']").val(),type:data.field.type,users:milasUrl}

            $.ajax({
                url: Feng.ctxPath + "/teams/addTeamUser?flag="+(data.field.show==0?false:true),
